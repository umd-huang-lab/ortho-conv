...
from utils import _power_iteration, _bjorck_orthonormalize
...
if __name__ == "__main__":
    ...
    for (in_features, out_features) in [(24, 12), (12, 24), (16, 16)]:
        for init in ["identical", "reverse", "permutation"]:
            ...
            module  = BjorckLinear(in_features = in_features, out_features = out_features, init = init,
                bjorck_order = bjorck_order, bjorck_iters = bjorck_iters, bjorck_thres = bjorck_thres,
                power_iters = power_iters, power_thres = power_thres).to(device)
            outputs = module(inputs, inverse = False, cache = False)

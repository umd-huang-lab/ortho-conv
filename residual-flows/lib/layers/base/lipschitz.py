...
from lconvnet.layers import LipschitzLinear, LipschitzConv2d
from lconvnet.layers import Paraunitary, BCOP, BjorckLinear, \
                            CayleyLinear, CayleyConv2d, \
                            skew_conv2d as SOC, skew_linear as ExpLinear
...
class SpectralNormLinear(nn.Module):
    ...
    def __init__(
        ...
        coeff = 0.97, n_iterations = None, atol = None, rtol = None, **unused_kwargs
    ):
        del unused_kwargs
...
def get_linear(in_features, out_features, bias = True, 
               coeff = 0.97, linear = 'SpectralNormLinear', 
               domain = None, codomain = None, **kwargs
):
    if domain == 2 and codomain == 2:
        if linear == 'SpectralNormLinear':
            return SpectralNormLinear(in_features, out_features, bias, coeff, **kwargs)
        else: # if linear != 'SpectralNormLinear':
            return LipschitzLinear(in_features, out_features, bias, coeff, eval(linear))

    if domain is None and domain is None:
...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, bias = True, 
               coeff = 0.97, conv2d = 'SpectralNormConv2d', 
               domain = None, codomain = None, **kwargs
):
    if domain == 2 and codomain == 2:
        if conv2d == 'SpectralNormConv2d':
            return SpectralNormConv2d(in_channels, out_channels, kernel_size, stride, padding, bias, coeff, **kwargs)
        else: # if conv2d != 'SpectralNormConv2d':
            return LipschitzConv2d(in_channels, out_channels, kernel_size, stride, padding, bias, coeff, eval(conv2d))

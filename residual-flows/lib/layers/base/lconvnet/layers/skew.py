...
class skew_linear(nn.Module):
    def __init__(self, in_features, out_features, 
                 bias=True, train_terms=5, eval_terms=10, init_iters=50, update_iters=1, 
                 update_freq=200, correction=0.7):
        super(skew_linear, self).__init__()

        self.linear = skew_conv2d(in_features, out_features, kernel_size = 1, stride = 1, padding = None,
            bias = bias, train_terms = train_terms, eval_terms = eval_terms, init_iters = init_iters, update_iters = update_iters,
            update_freq = update_freq, correction = correction)

    def forward(self, x):
        return self.linear(x.view(*x.shape, 1, 1)).squeeze()

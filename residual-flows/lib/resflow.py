...
class ResidualFlow(nn.Module):
    ...
    def __init__(
        ...
        coeff=0.9,
        vnorms='122f',
        linear='SpectralNormLinear',
        conv2d='SpectralNormConv2d',
    ...
    def __init__(
        ...
        def _resblock(initial_size, fc, idim=idim, first_resblock=False):
            if fc:
                ...
                nnet.append(
                    _lipschitz_layer(fc)(
                        initial_size[0], idim, ks[0], 1, ks[0] // 2, coeff=coeff, linear=linear, conv2d=conv2d,
                        n_iterations=n_lipschitz_iters, domain=_domains[0], codomain=_codomains[0], atol=sn_atol, rtol=sn_rtol
                    )
                )
                ...
                for i, k in enumerate(ks[1:-1]):
                    nnet.append(
                        _lipschitz_layer(fc)(
                            idim, idim, k, 1, k // 2, coeff=coeff, linear=linear, conv2d=conv2d, 
                            n_iterations=n_lipschitz_iters, domain=_domains[i + 1], codomain=_codomains[i + 1], atol=sn_atol, rtol=sn_rtol
                        )
                    )
                    if batchnorm: nnet.append(layers.MovingBatchNorm2d(idim))
                    nnet.append(ACT_FNS[activation_fn](True))
                ...
                nnet.append(
                    _lipschitz_layer(fc)(
                        idim, initial_size[0], ks[-1], 1, ks[-1] // 2, coeff=coeff, linear=linear, conv2d=conv2d,
                        n_iterations=n_lipschitz_iters, domain=_domains[-1], codomain=_codomains[-1], atol=sn_atol, rtol=sn_rtol
                    )
                )
                if batchnorm: nnet.append(layers.MovingBatchNorm2d(initial_size[0]))
                return layers.iResBlock(
...
class FCNet(nn.Module):
    ...
    def __init__(
        self, input_shape, idim, lipschitz_layer, nhidden, coeff, linear, domains, codomains, n_iterations, activation_fn,
        ...
        for i in range(nhidden):
            nnet.append(
                lipschitz_layer(last_dim, idim) if lipschitz_layer == nn.Linear else lipschitz_layer(
                    last_dim, idim, coeff=coeff, linear=linear,
                    n_iterations=n_iterations, domain=domains[i], codomain=codomains[i],
        ...
        nnet.append(
            lipschitz_layer(last_dim, dim) if lipschitz_layer == nn.Linear else lipschitz_layer(
                last_dim, dim, coeff=coeff, linear=linear,
                n_iterations=n_iterations, domain=domains[-1], codomain=codomains[-1],
                atol=sn_atol, rtol=sn_rtol
            )
        )

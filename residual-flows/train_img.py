...
import sys
sys.path.append('lib/layers/base')
import lconvnet
...
parser.add_argument('--linear', type = str, default = 'SpectralNormLinear', 
                    choices = ['SpectralNormLinear', 'BjorckLinear', 'CayleyLinear', 'ExpLinear'])
parser.add_argument('--conv2d', type = str, default = 'SpectralNormConv2d',
                    choices = ['SpectralNormConv2d', 'Paraunitary', 'BCOP', 'CayleyConv2d', 'SOC'])
...
parser.add_argument('--batchsize', help='Minibatch size', type=int, default=8)
...
if (args.resume is not None):
    ...
    with torch.no_grad():
        ...
        model(x)
    checkpt = torch.load(args.resume, pickle_module=dill)
...
def main():
    ...
    for epoch in range(args.begin_epoch, args.nepochs):
        ...
        torch.save({
            ...
            'ema': ema,
            'test_bpd': test_bpd,
        }, os.path.join(args.save, 'models', 'most_recent.pth'), pickle_module=dill)

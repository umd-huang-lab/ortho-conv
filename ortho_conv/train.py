...
parser.add_argument('--thres', default = 1e-6, type = float)
...
parser.add_argument('--conv',   default = 'CayleyConv',
                    choices = ['PlainConv', 'BjorckConv', 'CayleyConv', 'LieExpConv',
...
if args.model in ['KWLarge', 'ResNet9']:
    ...
    model = _model(conv = _conv, linear = _full, k = args.kernel)
    ...
    model = _model(conv = _conv, linear = _full,
        kernel_size = args.kernel, num_layers = args.layers)

elif args.model in ['ResNet', 'ShuffleNet', 'ConvNet']:
    model_name = args.model + str(args.layers)

    model = _model(conv = _conv, linear = _full, init = args.init,
        num_layers = args.layers, downsampling = args.downsampling,
        conv_kernel = args.kernel, conv_dilate = args.dilate, thres = args.thres)
...
# loss: multi-margin or cross_entropy
if args.ce_loss: 

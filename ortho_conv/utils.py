...
def cert_stats_(model, data_loader, cert):
    cert_right = torch.tensor(0.).cuda()
    cert_wrong = torch.tensor(0.).cuda()
    insc_right = torch.tensor(0.).cuda()
    insc_wrong = torch.tensor(0.).cuda()
    ...
    for X, y in data_loader:
        ...
        cert_right += torch.sum(correct & certified)
        cert_wrong += torch.sum(~correct & certified)
        insc_right += torch.sum(correct & ~certified)
        insc_wrong += torch.sum(~correct & ~certified)
...
def rob_acc_(model, data_loader, eps, pgd_alpha, opt, half_prec, attack_iters, n_restarts, rs=True, linf_proj=True,
            l2_grad_update=False, corner=False, print_fosc=False, verbose=False, cuda=True, lossf=F.cross_entropy):
    robust_acc = torch.tensor(0.).cuda()
    ...
    for (X, y) in data_loader:
        ...
        robust_acc += (output.max(1)[1] == y).sum()
